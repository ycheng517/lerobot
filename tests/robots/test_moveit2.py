...
def test_send_action_with_max_relative_target(moveit2_robot):
    ...
    action = {
        "linear_x": 1.0,  # Large value that should be clipped
        "linear_y": 0.0,
        "linear_z": 0.0,
        "angular_x": 0.0,
        "angular_y": 0.0,
    ...
    assert returned_action["linear_x"] <= 0.1
    assert returned_action["gripper"] <= 0.1
...
def test_keyboard_action_conversion(moveit2_robot):
    ...
    assert returned_action["linear_x"] == 1.0  # 'd' key
    assert returned_action["linear_y"] == 1.0  # 'w' key
    assert returned_action["linear_z"] == 0.0
    assert returned_action["gripper"] == 1.0  # 'space' key
...
def test_from_keyboard_to_action(moveit2_robot):
    ...
    pressed_keys = {
        "w": True,  # +linear_y
        "s": True,  # -linear_y (should cancel with w)
        "a": True,  # -linear_x
        "d": True,  # +linear_x (should cancel with a)
        "q": True,  # -linear_z
        "e": True,  # +linear_z (should cancel with q)
        "i": True,  # +angular_x
        "k": True,  # -angular_x (should cancel with i)
        "j": True,  # -angular_y
        "l": True,  # +angular_y (should cancel with j)
        "u": True,  # +angular_z
        "o": True,  # -angular_z (should cancel with u)
        "space": True,  # gripper
    }
    ...
    assert action["linear_x"] == 0.0
    assert action["linear_y"] == 0.0
    assert action["linear_z"] == 0.0
    assert action["angular_x"] == 0.0
    assert action["angular_y"] == 0.0
    assert action["angular_z"] == 0.0
    ...
    single_key_tests = [
        ...
        ({"q": True}, {"linear_z": -1.0}),
        ({"e": True}, {"linear_z": 1.0}),
        ({"i": True}, {"angular_x": -1.0}),
        ({"k": True}, {"angular_x": 1.0}),
        ({"j": True}, {"angular_y": -1.0}),
        ({"l": True}, {"angular_y": 1.0}),
        ({"u": True}, {"angular_z": 1.0}),
        ({"o": True}, {"angular_z": -1.0}),
    ]
...
def test_error_handling_when_not_connected(moveit2_robot):
    ...
    with pytest.raises(DeviceNotConnectedError):
        moveit2_robot.send_action({"linear_x": 0.1, "gripper": 0.5})
